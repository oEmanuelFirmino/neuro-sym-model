...
class TensorTestFormatter:
    ...
    def _setup_logger(self, log_level):
        logger = logging.getLogger("TensorTest")
    ...
    def print_operation_result(
        ...
        if hasattr(result, "shape") and hasattr(result, "data"):
            ...
            if isinstance(result, float):
                self.logger.info(f"   Result: {result:.4f}")
            else:
    ...
    def _format_recursive(self, data: Any, indent_level: int = 0) -> str:
        ...
        if isinstance(data, list):
            ...
            if not isinstance(data[0], list):
                formatted_nums = [f"{x:7.3f}" for x in data]
    ...
    def print_footer(self):
        ...
        self.logger.info("")
        self.logger.info(separator)
        self.logger.info("✅ Todos os testes executados com sucesso!")
        self.logger.info("📝 Tensor operations completed successfully")
        self.logger.info(separator)
        self.logger.info("")
...
class TensorTestSuite:
    ...
    def run_basic_operations_tests(self, t1, t2):
        self.formatter.print_section_header("Operações Básicas")
    ...
    def run_mathematical_functions_tests(self, t1):
        self.formatter.print_section_header("Funções Matemáticas")
    ...
    def run_all_tests(self):
        self.formatter.print_banner("Demonstração Completa da Classe Tensor")

        self.formatter.print_section_header("Inicialização dos Tensores")
        t1 = Tensor([[1.0, 2.0], [3.0, 4.0]])
        t2 = Tensor([[5.0, 6.0], [7.0, 8.0]])

        self.formatter.print_tensor_info("t1", t1, "Tensor 2x2 - Matriz A")
        self.formatter.print_tensor_info("t2", t2, "Tensor 2x2 - Matriz B")

        self.run_basic_operations_tests(t1, t2)
        self.run_mathematical_functions_tests(t1)
        self.run_reduction_operations_tests(t1)

        self.formatter.print_section_header("Teste com Tensor Escalar")
        t_scalar = Tensor(5.0)
        self.formatter.print_tensor_info("t_scalar", t_scalar, "Tensor escalar")
        ...
        self.formatter.print_section_header("Teste com Tensor 1D")
        t_1d = Tensor([1.0, 2.0, 3.0, 4.0])
        self.formatter.print_tensor_info("t_1d", t_1d, "Tensor 1D - Vetor")
...
def main():
    try:
        ...
        test_suite = TensorTestSuite(formatter)
